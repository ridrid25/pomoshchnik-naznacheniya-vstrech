  ...
  async submitDecision(
    ...
    try {
      ...
      const calendarUrl = booking
        ...
        : null;
      const miniAppUrl = booking ? createCalendarReturnUrl(booking.id) : null;
      const content = `${booking ? bookingCard(booking) : ''}${decisionResult(result.outcome)}`;
      this.send(
        response,
        200,
        page('Решение сохранено', content, calendarUrl, miniAppUrl),
      );
    } catch {
      this.send(
        response,
