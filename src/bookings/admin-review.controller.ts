...
export class AdminReviewController {
  ...
  async submitDecision(
    ...
    try {
      ...
      this.send(response, 200, page('Решение сохранено', content));
    } catch {
      this.send(
        response,
        500,
...
function decisionResult(outcome: BookingDecisionOutcome): string {
  ...
  return `<div class="result ${result[outcome][0]}">${result[outcome][1]}</div><p class="hint">Можно закрыть эту вкладку и вернуться в Google Calendar.</p>`;
}
