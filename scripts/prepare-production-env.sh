...
awk -F= '
  /^(NODE_ENV|PORT|LOG_LEVEL|DATABASE_URL|TELEGRAM_WEBHOOK_SECRET|TELEGRAM_DEV_POLLING|TELEGRAM_API_ROOT|GOOGLE_OAUTH_REDIRECT_URI|PUBLIC_BASE_URL|ADMIN_ACTION_SECRET|MINI_APP_SESSION_SECRET|MINI_APP_SESSION_TTL_SECONDS|MINI_APP_INIT_DATA_MAX_AGE_SECONDS|DOMAIN|APP_BIND_PORT)=/ { next }
...
mini_app_session_secret=$(openssl rand -hex 32)
printf '\nDOMAIN=%s\nAPP_BIND_PORT=%s\nNODE_ENV=production\nPORT=3000\nLOG_LEVEL=log\nDATABASE_URL=file:/app/data/app.db\nTELEGRAM_WEBHOOK_SECRET=%s\nTELEGRAM_DEV_POLLING=false\nTELEGRAM_API_ROOT=\nGOOGLE_OAUTH_REDIRECT_URI=https://%s/google/oauth/callback\nPUBLIC_BASE_URL=https://%s\nADMIN_ACTION_SECRET=%s\nMINI_APP_SESSION_SECRET=%s\nMINI_APP_SESSION_TTL_SECONDS=7200\nMINI_APP_INIT_DATA_MAX_AGE_SECONDS=600\n' \
  "$domain" "$app_bind_port" "$webhook_secret" "$domain" "$domain" "$admin_action_secret" "$mini_app_session_secret" >> "$target_file"
