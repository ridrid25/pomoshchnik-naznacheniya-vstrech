...
M8 Mini App: пилот и UX-полировка. Первый пакет обратной связи разделяет демонстрационную и настоящую заявки, объясняет появление бледного события «На согласовании» в Google Calendar и показывает, как вернуться к открытой карточке через соседнюю вкладку «Запись на встречу».
