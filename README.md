...
M8 Mini App: реальный пилот и UX-полировка. Деморежим и mock-заявки удалены; Mini App запускается только с подтверждённой Telegram-сессией, а каждая отправленная заявка проходит рабочий API и календарный цикл.
