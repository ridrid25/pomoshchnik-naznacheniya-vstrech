...
M7 Mini App: в карточку согласования добавлена проверка конкретного дня в Google Calendar. Администратор открывает календарь с серыми временными заявками, возвращается к той же карточке и подтверждает или отклоняет встречу. Пользовательский и административный интерфейсы собраны в тот же Docker-образ, используют общую бизнес-логику и защищённую Telegram-сессию.
