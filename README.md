...
Этап 7: бот работает на VPS через HTTPS webhook. Реализован полный жизненный цикл заявок — резервирование, подтверждение, отмена, перенос, TTL, выбор онлайн/личной встречи, Google Calendar/Meet и маршрутизация уведомлений через Telegram или SMTP Email. Необработанная заявка сразу показывается владельцу в Google Calendar как бледное событие `⏳ На согласовании`, которое не блокирует занятость. В описании события есть защищённая ссылка на страницу «Подтвердить / Отклонить»; те же действия остаются в Telegram. Подтверждение превращает серое событие в обычную встречу, отклонение удаляет.
...
`npm test` выполняет проверки Этапов 1–7: собирает приложение, проверяет HTTP endpoints, прогоняет полный bot-flow через локальный mock Telegram API, проверяет чистую SQLite-базу и резервную копию, расписание, Google-интеграцию, временные календарные маркеры, подписанную web-страницу решения, защиту от подмены и просроченной ссылки, перенос встреч, маршрутизацию уведомлений с повторными попытками и production-артефакты.
