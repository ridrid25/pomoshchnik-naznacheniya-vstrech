...
M9 Mini App завершён: очередь администратора проверяет занятость предложенного времени в реальном времени, помогает повторно выбрать слот, контролирует задержавшиеся решения и автоматически сравнивает новый контрольный поток с базовым пилотом M8.
