...
M6 Mini App: пользовательский и административный интерфейсы собраны в тот же Docker-образ, открываются по `/mini-app`, используют общую бизнес-логику и защищённую Telegram-сессию. Бот добавляет кнопку `✨ Открыть приложение`, а при старте настраивает стандартный Telegram menu button на production HTTPS URL. GitHub Actions проверяет backend, HTML, JavaScript и закрытый API перед завершением автодеплоя.
