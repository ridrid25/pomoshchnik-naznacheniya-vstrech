...
В Telegram должен использоваться только этот HTTPS URL. Demo-параметр `?demo=1` предназначен исключительно для локальной проверки и не должен добавляться в кнопку бота.
...
Стандартная кнопка меню настраивается автоматически через Bot API. Дополнительную крупную кнопку запуска в профиле можно включить отдельно в `@BotFather`, указав тот же URL без `?demo=1`. Это не заменяет серверную проверку `initData` и не требует другого frontend URL.
