...
- Разделы «Мои записи» и админка подключены к рабочему API на этапах M4–M5; деморежим удалён на этапе M8.
...
```text
http://127.0.0.1:3000/mini-app
```

Страница требует подтверждённую Telegram-сессию. Без неё показывается только безопасный экран запуска через production-бота `@Zapiscalender_bot`; mock-данные и обход через query-параметр отсутствуют.
