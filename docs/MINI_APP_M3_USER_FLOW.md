...
Страница требует подтверждённую Telegram-сессию. Без неё показывается только безопасный экран запуска через `@Zapiscalendar_bot`; mock-данные и обход через query-параметр отсутствуют.
